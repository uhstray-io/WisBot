...
- **WebService.cs** — ASP.NET Core / Kestrel web host. Routes: `GET /health` (200 connected / 503 starting); `GET /u/{id}` (upload form when pending, download landing when ready); `POST /u/{id}` (streamed file upload to MinIO, one per link); `GET /u/{id}/file` (download, forced attachment). Request limits raised to `WISBOT_UPLOAD_MAX_BYTES`. Bound to `WISBOT_HEALTH_HOST`/`WISBOT_HEALTH_PORT`. Started from `Bot.StartBot`.
- **UploadService.cs** — Phase 8 file relay. `/upload` mints an unguessable (128-bit base64url) link backed by a `pending` row in the `uploads` table (reply ephemeral). Also owns MinIO storage: `StoreAsync` streams the file into the `WISBOT_MINIO_BUCKET` and marks the row `ready` (filename/type/size/expiry); `DownloadToAsync` streams it back; `GetUploadAsync` reads metadata. An hourly retention loop (`StartRetention`, started in `OnReady`) deletes expired uploads (object + row) past `expires_at`. MinIO config via `WISBOT_MINIO_*`.
