...
- **UploadService.cs** — Phase 8 file relay. `/upload` mints an unguessable (128-bit base64url) link backed by a `pending` row in the `uploads` table (reply ephemeral). Also owns MinIO storage: `StoreAsync` streams the file into the `WISBOT_MINIO_BUCKET` and marks the row `ready` (filename/type/size/expiry); `DownloadToAsync` streams it back; `GetUploadAsync` reads metadata. MinIO config via `WISBOT_MINIO_*`.
